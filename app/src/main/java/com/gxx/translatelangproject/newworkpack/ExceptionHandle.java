...
public class ExceptionHandle {
    ...
    public static ResponeThrowable handleException(Throwable e) {
        ...
        if (e instanceof HttpException) {
           ...
           /*
           //如果后台有返回类似400的错误，然后也一并返回了错误信息，可以在这里进行错误信息处理
           try {
                String errString = httpException.response().errorBody().string();
                if (!StringUtils.isEmpty(errString)){
                    ex.message = errString;
                    return ex;
                }
            ...
            ex = new ResponeThrowable(e, ERROR.NOADDRESS_ERROR);
            ex.message = "请求地址不对";
            return ex;
        }
        else {
